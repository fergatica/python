    ...
    try:
        with open(filename, mode="r") as fileobj:
            for line in fileobj:
                ...
                counter1 = 1
                counter2 = len(fields) - 1
                key_name = fields[0]

                while counter2 > 0:
                    if key_name in network:
                        if (fields[counter1] not in network[key_name]) and (fields[counter1] != key_name):
                            network[key_name].append(fields[counter1])
                        if key_name not in network[fields[counter1]] and (key_name != fields[counter1]):
                            network[fields[counter1]].append(key_name)
                        counter1 += 1
                        counter2 -= 1
...
def add_friendship(network, name1, name2):
    ...
    if name1 == name2:
        ...
        if name1 in network and name2 in network[name1]:
            return

        if name1 in network:
            network[name1].append(name2)
        elif name1 not in network:
            network[name1] = [name2]
...
def print_command(network, namelist):
    ...
    for key in sorted(network):
        print(key)
        for value in sorted(network[key]):
            print("-", value)
...
def common_friends_command(network, namelist):
    ...
    list_of_common_friends = set(network[name1]) & set(network[name2])

    if len(list_of_common_friends) < 1:
        print(name1, "and", name2, "have no common friends.")
        return
    for item in sorted(list_of_common_friends):
        print(item)
